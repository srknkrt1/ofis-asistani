...
def generate_video():
    ...
    # İlk sütun kategoriler (örneğin "Kategori 1", "Kategori 2")
    category_col = df.columns[0]
    df.set_index(category_col, inplace=True)
    df = df.apply(pd.to_numeric, errors='coerce')  # Sayısallaştırma
    ...
    try:
        ...
        return jsonify({"error": f"Video oluşturulurken bir hata oluştu: {str(e)}"})

    return jsonify({"video_url": f"/downloads/{video_filename}"})
